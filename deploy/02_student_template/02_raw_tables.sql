...
COPY INTO {MUNICIPALITY}_DB.RAW.STUDENTS_RAW
FROM @{MUNICIPALITY}_DB.RAW.{MUNICIPALITY}_STAGE/
PATTERN = '.*dim_students.*\.csv';

COPY INTO {MUNICIPALITY}_DB.RAW.TEACHERS_RAW
FROM @{MUNICIPALITY}_DB.RAW.{MUNICIPALITY}_STAGE/
PATTERN = '.*dim_teachers.*\.csv';

COPY INTO {MUNICIPALITY}_DB.RAW.CLASSES_RAW
FROM @{MUNICIPALITY}_DB.RAW.{MUNICIPALITY}_STAGE/
