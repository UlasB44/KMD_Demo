...
-- Replace aalborg with your assigned municipality (lowercase)
...
COPY INTO AALBORG_DB.RAW.STUDENTS_RAW
FROM @AALBORG_DB.RAW.AALBORG_STAGE/
PATTERN = '.*dim_students.*\.csv';

COPY INTO AALBORG_DB.RAW.TEACHERS_RAW
FROM @AALBORG_DB.RAW.AALBORG_STAGE/
PATTERN = '.*dim_teachers.*\.csv';

COPY INTO AALBORG_DB.RAW.CLASSES_RAW
FROM @AALBORG_DB.RAW.AALBORG_STAGE/
