...
-- Replace aarhus with your assigned municipality (lowercase)
...
COPY INTO AARHUS_DB.RAW.STUDENTS_RAW
FROM @AARHUS_DB.RAW.AARHUS_STAGE/
PATTERN = '.*dim_students.*\.csv';

COPY INTO AARHUS_DB.RAW.TEACHERS_RAW
FROM @AARHUS_DB.RAW.AARHUS_STAGE/
PATTERN = '.*dim_teachers.*\.csv';

COPY INTO AARHUS_DB.RAW.CLASSES_RAW
FROM @AARHUS_DB.RAW.AARHUS_STAGE/
