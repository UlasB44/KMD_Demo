...
-- Replace copenhagen with your assigned municipality (lowercase)
...
COPY INTO COPENHAGEN_DB.RAW.STUDENTS_RAW
FROM @COPENHAGEN_DB.RAW.COPENHAGEN_STAGE/
PATTERN = '.*dim_students.*\.csv';

COPY INTO COPENHAGEN_DB.RAW.TEACHERS_RAW
FROM @COPENHAGEN_DB.RAW.COPENHAGEN_STAGE/
PATTERN = '.*dim_teachers.*\.csv';

COPY INTO COPENHAGEN_DB.RAW.CLASSES_RAW
FROM @COPENHAGEN_DB.RAW.COPENHAGEN_STAGE/
