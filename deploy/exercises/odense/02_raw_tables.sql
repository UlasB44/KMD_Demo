...
-- Replace odense with your assigned municipality (lowercase)
...
COPY INTO ODENSE_DB.RAW.STUDENTS_RAW
FROM @ODENSE_DB.RAW.ODENSE_STAGE/
PATTERN = '.*dim_students.*\.csv';

COPY INTO ODENSE_DB.RAW.TEACHERS_RAW
FROM @ODENSE_DB.RAW.ODENSE_STAGE/
PATTERN = '.*dim_teachers.*\.csv';

COPY INTO ODENSE_DB.RAW.CLASSES_RAW
FROM @ODENSE_DB.RAW.ODENSE_STAGE/
